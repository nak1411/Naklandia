...
func update_thumbnails() -> void:
	if not is_instance_valid(plugin.terrain):
		return
	if _current_list.type == Terrain3DAssets.TYPE_MESH and Time.get_ticks_msec() - _last_thumb_update_time > MAX_UPDATE_TIME:
...
class ListContainer:
	...
	func set_selected_id(p_id: int) -> void:
		...
		if type == Terrain3DAssets.TYPE_TEXTURE and not plugin.editor.get_tool() in [Terrain3DEditor.TEXTURE, Terrain3DEditor.COLOR, Terrain3DEditor.ROUGHNESS]:
	...
	func redraw() -> void:
		...
		for c in get_children():
			if is_instance_valid(c):
				...
				c.position = Vector2(id % columns, id / columns) * width + Vector2(separation / columns, separation / columns)
