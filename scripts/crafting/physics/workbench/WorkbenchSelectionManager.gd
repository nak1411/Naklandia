...
var selection_pivot_point: Vector3 = Vector3.ZERO  # Average center of selected items
...
func _init(p_camera: Camera3D, p_viewport: SubViewport, p_viewport_container: SubViewportContainer, p_world: Node3D, p_selection_overlay: Control) -> void:
...
func try_select_single(_mouse_pos: Vector2, modifier_held: bool) -> void:
	...
	if result and result.collider is PhysicalItem:
		...
		if modifier_held:
			...
			if item in selected_items:
				...
				select_item(item, true)  # Add to selection
		else:
...
func try_box_select(shift_held: bool = false, ctrl_held: bool = false) -> void:
	...
	if items_to_select.size() > 0:
		...
		# Shift = add to selection, Normal = replace selection
			...
			# Update pivot after bulk selection
			_update_selection_pivot()
...
func deselect_item(item: PhysicalItem) -> void:
	...
	if item in selected_items:
		...
		print("Deselected: %s" % item.item_name)
		_update_selection_pivot()
	selection_changed.emit(selected_items)
...
func clear_selection() -> void:
	...
	selection_pivot_point = Vector3.ZERO
	selection_changed.emit(selected_items)
...
func _update_selection_pivot() -> void:
	"""Calculate the average center of all selected items as the pivot point."""
	if selected_items.is_empty():
		selection_pivot_point = Vector3.ZERO
		return

	# Simple average of all item positions
	selection_pivot_point = Vector3.ZERO
	for item in selected_items:
		selection_pivot_point += item.global_position

	selection_pivot_point /= selected_items.size()
	print("Selection pivot updated to: %.2f, %.2f, %.2f (from %d items)" % [selection_pivot_point.x, selection_pivot_point.y, selection_pivot_point.z, selected_items.size()])
