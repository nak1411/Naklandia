...
func add_menu_item(id: String, text: String, has_submenu: bool = false, submenu_items: Array = []):
	var item = {"id": id, "text": text, "has_submenu": has_submenu, "submenu_items": submenu_items, "enabled": true}
...
func _show_submenu(item_index: int):
	...
	var submenu_pos = Vector2i(main_popup.position.x + menu_width, main_popup.position.y + (item_index * item_height))
...
func setup_window_options_menu():
	...
	for i in range(10, 101, 10):
		transparency_items.append({"id": "transparency_" + str(i), "text": str(i) + "%"})
