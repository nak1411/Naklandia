...
func refresh_from_inventory():
	"""Called externally when inventory changes"""
	if not player_inventory:
		return

	# Force refresh of quick slots from current inventory state
	_refresh_quick_slots()
...
func _show_item_info(item: InventoryItem_Base):
	...
	tooltip.dialog_text = "Type: %s\nQuantity: %d\nValue: %.2f ISK" % [ItemTypes.Type.keys()[item.item_type], item.quantity, item.get_total_value()]
...
func save_quick_slot_config() -> Dictionary:
	var config = {"selected_slot": selected_slot_index, "slot_items": []}

	for i in quick_slots.size():
		var item = quick_slots[i].get_item()
		if item:
			config.slot_items.append({"slot_index": i, "item_id": item.item_id})
		else:
