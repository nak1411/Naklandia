# InventoryListView.gd - List view for inventory (Eve Online style)
...
# List management
var item_rows: Array[ListRowManager] = []
...
func _on_row_clicked(row: ListRowManager, item: InventoryItem_Base, event: InputEvent):
	if event is InputEventMouseButton:
...
func _on_row_double_clicked(_row: ListRowManager, _item: InventoryItem_Base):
	pass


func _on_row_right_clicked(_row: ListRowManager, item: InventoryItem_Base, _position: Vector2):
...
func drop_data(_position: Vector2, data: Variant):
	...
	var success = inventory_manager.transfer_item(source_item, source_container_id, container.container_id, Vector2i(-1, -1))  # Let container decide position
...
func _handle_drop_on_empty_area(drag_data: Dictionary, _drop_position: Vector2) -> bool:
	...
	var success = inventory_manager.transfer_item(source_item, source_container_id, container_id, Vector2i(-1, -1))
