...
func _apply_current_sort():
	...
	match sort_type:
		InventorySortType.Type.BY_NAME:
			view_items.sort_custom(func(a, b): 
				return a.item_name.naturalnocasecmp_to(b.item_name) < 0 if sort_ascending else a.item_name.naturalnocasecmp_to(b.item_name) > 0
			)
		InventorySortType.Type.BY_TYPE:
			view_items.sort_custom(func(a, b): 
				return a.item_type < b.item_type if sort_ascending else a.item_type > b.item_type
			)
		InventorySortType.Type.BY_VALUE:
			view_items.sort_custom(func(a, b): 
				return a.get_total_value() > b.get_total_value() if sort_ascending else a.get_total_value() < b.get_total_value()
			)
		InventorySortType.Type.BY_VOLUME:
			view_items.sort_custom(func(a, b): 
				return a.get_total_volume() > b.get_total_volume() if sort_ascending else a.get_total_volume() < b.get_total_volume()
			)
