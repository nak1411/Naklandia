...
# EVE Online inspired color schemes
...
static func lighten_color(color: Color, factor: float) -> Color:
	return Color(
		mini(color.r + factor, 1.0),
		mini(color.g + factor, 1.0), 
		mini(color.b + factor, 1.0),
		color.a
	)

static func darken_color(color: Color, factor: float) -> Color:
	return Color(
		maxi(color.r - factor, 0.0),
		maxi(color.g - factor, 0.0),
		maxi(color.b - factor, 0.0), 
		color.a
	)
