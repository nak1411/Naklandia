...
static func is_valid_item(item: InventoryItem_Base) -> bool:
	...
	return (
		not item.item_name.is_empty() and
		item.volume > 0.0 and
		item.mass >= 0.0 and
		item.quantity > 0 and
...
static func can_stack_items(item1: InventoryItem_Base, item2: InventoryItem_Base) -> bool:
	...
	return (
		item1.item_id == item2.item_id and
		item1.max_stack_size > 1 and
		item2.max_stack_size > 1 and
		not item1.is_unique and
		not item2.is_unique
	)
...
static func validate_transfer(
	from_container: InventoryContainer_Base, 
	to_container: InventoryContainer_Base, 
	item: InventoryItem_Base,
	quantity: int = -1
) -> Dictionary:
	var result = {
		"valid": false,
